...
model2 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unempoy_rate + log(density), data = df)
model3 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unempoy_rate + log(density) + pct_lt17 + pct_gt65, data = df) #final model
...
1. **IID Sampling:** There isn't a direct database check for this unfortunately. Looking into how the data was generated, we believe this is IID generally. The data was gathered from independent surveys within the Texas population for all models generated.

2. **No Perfect Colinearity:** This can be assessed through the variance inflation factor (VIF). Running a VIF in R in the cell below, we see the VIF score is relatively low for most predictors excluding the transformed Population Density variable. Additionally, when we view the relationship 
