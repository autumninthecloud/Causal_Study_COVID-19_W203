...
For the analysis, we will create three models to understand if causal relationships exist between the outcome variable, the number of long distance trips, and the control variables per Texas county: income, population density, age, Covid-19 cases and deaths . We will use an indicator variable, rural counties, to compare performance of our models for both rural or urban counties. By creating models with these variables, we intend to answer our primary and secondary research questions to provide insight about how vacation travel has been impacted so that interested stakeholders can gain an understanding of how to improve the revenue declines in the travel industry.
...
table1::table1(~density + cases + unemployed + median_income + lt17 + gt65 | rural_cat, data = df, caption = "Table 1: Texas County Variable Characteristics", topclass="Rtable1-zebra", overall = "Total")
```
Below are histograms of variables that did not follow normal distribution and required transformation. Each of these variables were logarithmically transformed.
```{r scatterplots, message=FALSE, echo=FALSE}
attach(df)
par(mfrow=c(3,2))
hist(cases, main="Histogram of Covid-19 Cases")
hist(deaths, main="Histogram of Covid-19 Deaths")
hist(long_trip, main="Histogram of Trip Count")
...
Upon transformation, below is a condensed view of the statistics of each variable:
...
$Model_1: f(\text{log(#Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)}) + \beta_2 g(\text{log(Covid-19 Deaths)})  + \beta_3 h(\text{Rural County})$
...
$Model_2: f(\text{log(#Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)}) + \beta_2 g(\text{log(Covid-19 Deaths)})  + \beta_3 h(\text{Rural County}) + \beta_4 h(\text{Median Income}) + \beta_5 h(\text{County Density})$
...
$Model_3: f(\text{log(#Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)}) + \beta_2 g(\text{log(Covid-19 Deaths)})  + \beta_3 h(\text{Rural County}) + \beta_4 h(\text{Median Income}) + \beta_5 h(\text{County Density}) + \beta_6 h(\text{Population under 17}) + \beta_7 h(\text{Population over 65})$ 
...
model1 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + log(median_income+1), data = df) #limited model
model2 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + log(median_income) + unemp_rate + log(density), data = df)
model3 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + log(median_income)+ unemp_rate + log(density) + pct_lt17 + pct_gt65, data = df) #final model
...
* In model1, all of our variables are extremely significant, though in the opposite direction as expected. We would expect an inverse relationship between covid and travel, though it seems that this model is producing a positive relationship.
...
* In model3 with age factored in, we see that actually the most statistically significant effect on long trips are median income of the household and presence of kids under 17. 
...
* In model2, we see that density has a more positive relationship with trips, with every percent increase in county density modeled to a half percent increase in trips. Also the positive relationship between covid cases/ deaths and trips has decreased to resulting in 0.15 and 0.25% increase in trips respectively.
* In model3, covid cases and deaths become insignificant. The coefficients have become slightly negative as in increase in cases and deaths decreases trips, which is in line with our original hypothesis, but is not significant. Instead, for every percent median income of the county increases by, the amount of long trips decrease by 0.3%. This could indicate that more affluent counties trust science and err on the safe side in the midst of covid. Finally, in our 3rd model, counties with more people under 17 years of age are more likely to take long trips. Due to the time frame of our analysis, this also makes sense because june is the start of summer break when many families like to take vacations together.


# Model Limitations
>>>>>>> 6ca246330f02d2b1f157dc724cc9cf099142c8b0

DELTE*** Not CLM since >100 samples. Large sample assumptions = IID and unique BLP (which is related to colinearity...)
