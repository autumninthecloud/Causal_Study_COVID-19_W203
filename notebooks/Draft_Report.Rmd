...
df %>%
  ...
  select(long_trip, cases, deaths, median_income, povery_rate, density, pct_lt17, pct_gt65) %>%
  GGally::ggpairs(.,
               legend = 1,
               aes(color = County),
...
Here is the first model, which is limited to study only key variables of interest for measurement based on our hypotheses. Our key input variable is covid cases and key output is number of trips greater than 100 miles. We also chose to add an indicator for rural counties because it could be directly related to trip distance, as well as median income which plays a role in a person's ability to take vacations in the first place. 
$$
...
$$ 
The second model expands upon the first model to also include key explanatory variables and covariates that are hypothesized to improve performance of the previous model. Here, we added in unemployment rate to assess if the impact of covid to that county affected travel.
$$
...
model2 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + log(median_income+1) + povery_rate+ log(density), data = df)
model3 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + log(median_income+1) + povery_rate+ log(density) + pct_lt17 + pct_gt65, data = df) #final model
stargazer(model1, model2, model3, type='text', title="Results", align=TRUE)
```
From this output table, we can see most of our hypotheses about the variables we included in our model were correct and do have a statistically significant contribution to our model already. 
