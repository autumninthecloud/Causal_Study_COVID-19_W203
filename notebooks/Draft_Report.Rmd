...
hist(unemployed, main="Histogram of Unemployment Rate")
...
$Model_1: f(\text{log(Number of Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)})  + \beta_2 h(\text{Rural County}) + \beta_3 h(\text{Median Income})$


$Model_2: f(\text{log(Number of Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)})  + \beta_2 h(\text{Rural County}) + \beta_3 h(\text{Median Income}) + \beta_4 h(\text{Unemployment Rate}) + \beta_5 h(\text{log(Population Density})$

$Model_3: f(\text{log(Number of Long Trips)}) = \beta_0 + \beta_1 g(\text{log(Covid-19 Cases)})  + \beta_2 h(\text{Rural County}) + \beta_3 h(\text{Median Income}) + \beta_4 h(\text{Unemploymed}) + \beta_5 h(\text{log(Population Density}) + \beta_6 h(\text{log(Age < 17}) + \beta_7 h(\text{log(Age > 65}) + \beta_7 h(\text{log(In Poverty})$
...
model2 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unemployed + log(density), data = df)
model3 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unemployed + log(density) + lt17 + gt65, data = df) #final model
