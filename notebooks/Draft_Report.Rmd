...
df %>%
  ...
  select(long_trip, cases, deaths, median_income, unemp, density, pct_lt17) %>%
  GGally::ggpairs(.,
...
model2 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unemp + log(density), data = df)
model3 <- lm(log(long_trip) ~ log(cases+1) + log(deaths+1) + is_rural + median_income + unemp + log(density) + pct_lt17 + pct_gt65, data = df) #final model
